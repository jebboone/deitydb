...
Snapshot for picking the project up on another machine. Last updated **2026-06-20, v2.1.7**.
...
(measured 2026-06-20): **0 entities fully uncited**; **1,453 carry real verbatim
quotes**; the remaining surface is **1,152 `primary-uncited`** entities (primary text known,
...
## Current state
- **Scale:** 3,837 entities / 7,098 relationships / 458 sources / 135 traditions. Code & DB at **v2.1.7** on `main`.
- **Citation remediation (v2.1.0–2.1.7): IN PROGRESS** — `entity_citations` table + `v_public_entity_citations` view; every entity graded (`primary-verbatim` / `primary-uncited` / `secondary` / `reference`), 0 fully uncited. **1,453 verbatim quotes** substring-gated against public-domain texts; 1,152 `primary-uncited` remain as upgrade targets. Pipeline + run order: `scripts/citations/README.md`. Triage/plan: `docs/REMEDIATION_TRIAGE.md`.
...
- **In git (clones cleanly):** all schema, `scripts/` (build SQL + `gen_*.py` generators + the `_p1..p7_*` cohort JSON + `scripts/citations/`), `templates/`, `static/`, `plugins/`, `metadata.yaml`, `docs/`, and portable Postgres dumps in `backups/` — **`deitydb_pg_v2.1.7.sql.gz` is current (includes the citation layer)**; `deitydb_pg_v2.0.0.sql.gz` is the pre-citation baseline kept for reference.
...
sleep 5
gunzip -c backups/deitydb_pg_v2.1.7.sql.gz | podman exec -i deitydb psql -U postgres -d deitydb
